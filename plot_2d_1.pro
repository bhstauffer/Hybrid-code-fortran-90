pro plot_image,img,x,y,sclf,loc,tit
   ...
   im = image(img,x,y,/current,rgb_table=33,layout=[3,3,loc],/buffer,font_size=12)
...
;                 position=[max(x), min(y), 0.05*(max(x)-min(x)),max(y)],/data)
   im.scale,sclf,sclf
...
pro plot_2d_1,nf,yslc
  ...
  dir = '/Volumes/Scratch/hybrid/KH_new/run_1/'
...
;wdims = w.window.dimensions
  ...
  for nfrm = 1,nframe,1 do begin
     ...
     bx = (reform(b1(*,yslc,*,0))*mp/q)/1e-9
     by = (reform(b1(*,yslc,*,1))*mp/q)/1e-9
     bz = (reform(b1(*,yslc,*,2))*mp/q)/1e-9

     binplane = sqrt(bx^2 + bz^2)
     
     nparr = reform(np(*,yslc,*))/1e15
     upx = reform(up(*,yslc,*,0))
     upy = reform(up(*,yslc,*,1))
     upz = reform(up(*,yslc,*,2))
     mixarr = reform(mixed(*,yslc,*))
     tparr = reform(tp(*,yslc,*))
     ...
     plot_image,binplane,x,z,sclf,3,'$B_{inplane}$ (nT)'
...
;;   p = plot(xp(*,0),xp(*,2),'w.',/overplot,/current,/buffer,axis_style=0)
     ...
     plot_image,tparr,x,z,sclf,7,'$T_p/<T_p>$'
     
     
     nparr = nparr*tparr/double(mp*nparr)^(5./3.)
     nparr = nparr/(total(nparr(*,50))/nx)
     
     plot_image,nparr,x,z,sclf,8,'$(p/\rho^\gamma)/<p/\rho^\gamma>$'
     ...
     plot_image,mixarr,x,z,sclf,9,'mixing'
  ...
  npim = image(nparr(*,50:nz-50),rgb_table=33,/buffer)
