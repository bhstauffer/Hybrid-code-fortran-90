...
ytext = OBJ_NEW('IDLgrText',['50','100','150','200'],baseline = [1.0,0,0],$
               alignment=0.5,/enable_formatting,color=!color.black,$
                char_dimensions = [5,5])

ztext = OBJ_NEW('IDLgrText',['10','20','30','40','50'],baseline = [1.0,0,0],$
               alignment=0.5,/enable_formatting,color=!color.black,$
              char_dimensions = [5,5])

xtext = OBJ_NEW('IDLgrText',['10','20','30','40','50','60'],baseline = [1.0,0,0],$
               alignment=0.5,/enable_formatting,color=!color.black,$
                char_dimensions = [5,5])

xax = OBJ_NEW('IDLgrAxis',direction=0,range=[minx,maxx],location=[0,0,0],$
              /exact,thick=3,ticklen=2,ticktext=xtext,tickdir=1)
yax = OBJ_NEW('IDLgrAxis',direction=1,range=[miny,maxy],location=[0,0,0],$
              /exact,thick=3,ticklen=2,ticktext=ztext,tickdir=1)
zax = OBJ_NEW('IDLgrAxis',direction=2,range=[minz,maxz],location=[0,0,0],$
              /exact,thick=3,ticklen=2,ticktext=ytext,tickdir=1)

omegapi = "167b
greekletter = '!9'+String(omegapi) + '!X'

xtit = OBJ_NEW('IDLgrText','x (c/'+greekletter+'!dpi!n)',baseline = [1.0,0,0], $
               locations=[nx/2,-20,0],$
               alignment=0.5,/enable_formatting,color=!color.black,$
              char_dimensions = [10,10])

ytit = OBJ_NEW('IDLgrText','z',baseline = [0,1.0,0], $
               updir = [-1.0,0,0], locations=[-15,ny/2,0],$
               char_dimensions=[10,10],$
               alignment=0.5,/enable_formatting,color=!color.black)


ztit = OBJ_NEW('IDLgrText','y (c/'+greekletter+'!dpi!n)',baseline = [0.0,0,1.0], $
               updir = [-1.0,0.0,0.0],locations=[-20,0,nz/2],$
               char_dimensions=[10,10],$
               alignment=0.5,/enable_formatting,color=!color.black)
